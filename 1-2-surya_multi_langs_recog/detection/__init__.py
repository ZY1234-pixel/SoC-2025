...
from detection.loader import DetectionModelLoader
from detection.parallel import FakeExecutor
from concurrent.futures import ThreadPoolExecutor
from detection.heatmap import parallel_get_boxes
from detection.schema import TextDetectionResult
from detection.util import get_total_splits, split_image
...
class DetectionPredictor(BasePredictor):
    ...
    def __call__(self, images: List[Image.Image], batch_size=None, include_maps=False) -> List[TextDetectionResult]:
        detection_generator = self.batch_detection(images, batch_size=batch_size,
                                                   static_cache=settings.DETECTOR_STATIC_CACHE)
    ...
    def batch_detection(
        ...
        for batch_idx in tqdm(range(len(batches)), desc="Detecting bboxes", disable=self.disable_tqdm):
            ...
            with settings.INFERENCE_MODE():
                pred = self.model(pixel_values=batch.to(
                    self.model.device))  # Moving the to device here fixes issues with xla recompilation
