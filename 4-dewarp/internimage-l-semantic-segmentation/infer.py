...
DEFAULT_INPUT = PACKAGE_DIR.parent / "TEST_dewarp"
DEFAULT_CONFIG = PACKAGE_DIR / "configs" / "docseg_internimage_l_1024.py"
DEFAULT_CHECKPOINT = PACKAGE_DIR / "checkpoints" / "best_final_before.pth"
DEFAULT_OUTPUT_DIR = PACKAGE_DIR / "outputs"


def resolve_checkpoint(checkpoint):
    path = Path(checkpoint).expanduser() if checkpoint else DEFAULT_CHECKPOINT
    path = path.resolve()
    if not path.is_file():
...
def restore_mask(mask_1024, meta):
    ...
    return cv2.resize(
        crop, (meta["w"], meta["h"]), interpolation=cv2.INTER_NEAREST)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run InternImage-L document-mask inference on 4-dewarp/TEST_dewarp by default.")
    parser.add_argument("--input", default=str(DEFAULT_INPUT), help="Image file or directory.")
    parser.add_argument(
        "--checkpoint",
        default=None,
        help="Defaults to checkpoints/best_final_before.pth.")
