...
from model_utils.utils_model import Conv_BN_ReLU
...
class LocalityAwareFeedforward(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.conv1 = Conv_BN_ReLU(d_in, d_hid, 1, 1, 0)

        self.depthwise_conv = Conv_BN_ReLU(d_hid, d_hid, 3, 1, 1, d_hid)

        self.conv2 = Conv_BN_ReLU(d_hid, d_in, 1, 1, 0)
...
class CrossattnLayer(nn.Module):
    ...
    def forward(self, x, cross, h, w, mask=None):
        n, hw, c = cross.size()
        residual = cross
        cross = self.norm1(cross)
        cross = residual + self.attn(cross, cross, cross, mask)
        residual = cross
        cross = self.norm2(cross)
        cross = residual + self.cross_attn(cross, x, x, mask)
        residual = cross
        cross = self.norm3(cross)
        cross = cross.transpose(1, 2).contiguous().view(n, c, h, w)
        cross = self.feed_forward(cross)
        cross = cross.view(n, c, hw).transpose(1, 2)
        cross = residual + cross
        return cross
...
class CrossAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.cross_layer_stack = nn.ModuleList([
            CrossattnLayer(
    ...
    def forward(self, feat, cross_feat, img_metas=None):
        ...
        for cross_enc_layer in self.cross_layer_stack:
            cross = cross_enc_layer(output, cross, h, w, mask)
        cross = self.layer_norm(cross)

        return cross


class SelfAttenLayer(nn.Module):
    """Implement encoder for SATRN, see `SATRN.
    <https://arxiv.org/abs/1910.04396>`_.
    Args:
        n_layers (int): Number of attention layers.
...
if __name__ == '__main__':
    ...
    encoder = CrossAttention(n_layers=8, n_position=28).cuda()
    decoder = SelfAttention(n_layers=4, n_position=28).cuda()
