...
from loader.dataset_doc3d_grid_HV import *

from model_utils.lr_scheduler import WarmupCosineLR
from networks.d2dewarp_model import *
...
def get_args():
    ...
    parser.add_argument('--save_interval', type=int, default=2500000, help='Save model interval between each train')
    parser.add_argument('--save_batch_interval', type=int, default=10, help='Save model interval between each train')
    parser.add_argument('--show_iter', type=int, default=5, help='Show log interval between each train')
    parser.add_argument('--batch_size', type=int, default=4, help='batch size')

    parser.add_argument('--exp_name', default='dewarp_HV_100K', help='Where to store logs and models')
    parser.add_argument('--data_path', default='UVDoc_final',
                        help='dataset path')

    parser.add_argument('--pre_trained_path', default='/home/tjq/PycharmProjects/D2Dewarp-main/model_weight.pt',
    ...
    parser.add_argument('--save_path', default="output", help='Save Model')
    ...
    parser.add_argument('--warmup_steps', type=int, default=0, help='steps to warmup LR')
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        with tqdm(total=train_batches) as pbar:
            ...
            while batch is not None:
                ...
                h_img, v_img, lbl, img, edge = batch[0], batch[1], batch[2], batch[3], batch[4]

                # 网络输入：直接使用二值掩码图像（1通道）
                pred_bm = model(h_img.float(),v_img.float(),img.float(),edge.float())
                # pred_h_lst, pred_v_lst, pred_bm = model(img.float(),edge.float())
                ...
                lr_scheduler.step()
        if epoch % parser.save_batch_interval == 0:
            torch.save(
...
if __name__ == '__main__':
    ...
    model = D2DewarpModel_my(img_size=parser.input_size, in_chans=parser.in_chans,
    ...
    dataset_train = Warp_DataSet_my(parser.input_size, data_path, split='UVDoc_final', is_aug=True)
    ...
    if parser.pre_trained_path:
        ...
        pretrained = torch.load(parser.pre_trained_path)
        ...
        print(f"✅ Loaded: {len(load_dict)} params")
        print(f"⚠️ Skipped: {len(skipped_keys)} params")
        for k in skipped_keys[:10]:  # 打印前 10 个跳过的 key
            print("  -", k)
