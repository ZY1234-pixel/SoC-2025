...
class D2DewarpModel_my(nn.Module):
    def __init__(self, img_size=448, in_chans=1, d_model=256):
        super().__init__()
        self.scale = 8
        self.d_model = d_model
        self.in_chans=in_chans
        self.unet_2decoder = UNetLineSeg(
            n_channels=self.in_chans,
            n_classes=1,
            d_model=d_model
        )
    ...
    def forward(self, h, v, img, edge):
        x = torch.cat([h, v, edge], dim=1)
        _, _, h_map, v_map = self.unet_2decoder(x)
...
if __name__ == '__main__':
    x = torch.randn((2, 1, 448, 448)).cuda()
    net = D2DewarpModel(img_size=448, in_chans=1, d_model=448).cuda()
