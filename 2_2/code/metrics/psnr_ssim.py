...
def calculate_psnr(img1,
    ...
    """计算峰值信噪比（PSNR）。
    参数：
        img1 (ndarray/tensor)：图像，取值范围为[0, 255]/[0, 1]。
        img2 (ndarray/tensor)：图像范围为[0, 255]/[0, 1]。
        crop_border (int)：图像每条边缘裁剪的像素数。这些像素不参与PSNR计算。
        input_order (str)：输入顺序为'HWC'或'CHW'。
            默认值：‘HWC’。
        test_y_channel (bool)：是否对 YCbCr 的 Y 通道进行测试。默认值：False。
    返回值：
        float：PSNR 结果。
    """
...
def _ssim(img1, img2):
    ...
    调用函数：`calculate_ssim`。
    参数：
        img1 (ndarray)：范围为[0, 255]、顺序为'HWC'的图像。
        img2 (ndarray)：范围为[0, 255]、顺序为'HWC'的图像。
    返回值：
        float：SSIM计算结果。
    """
...
def calculate_ssim(img1,
                   ...
                   test_y_channel=False):
    """计算SSIM
    对于三通道图像，先分别计算各通道的SSIM值，再取平均值。
    参数：
        img1 (ndarray)：图像，取值范围[0, 255]。
        img2 (ndarray)：图像，取值范围[0, 255]。
        crop_border (int)：图像每边裁剪像素数。这些像素不参与SSIM计算。
        input_order (str): 输入通道顺序为'HWC'或'CHW'。默认值：‘HWC’。
        test_y_channel (bool): 是否对YCbCr的Y通道进行测试。默认值：False。
    返回值：
        float: SSIM计算结果。
    """
