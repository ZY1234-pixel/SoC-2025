...
def get_transform(opt, params=None, grayscale=False, convert=True, additional_targets=None):
    ...
    # 三种模式：resize、resize_and_crop、crop_only（纯裁剪）
    if opt.preprocess == 'resize_and_crop':
        if params is None:
            transform_list.append(RandomResizedCrop(opt.crop_size, opt.crop_size, scale=(0.9, 1.0)))
    elif opt.preprocess == 'resize':
        transform_list.append(albumentations.Resize(opt.input_size, opt.input_size))
    elif opt.preprocess == 'crop_only':
        # 纯裁剪：不缩放，直接裁剪固定大小
        transform_list.append(albumentations.CenterCrop(opt.crop_size, opt.crop_size))
