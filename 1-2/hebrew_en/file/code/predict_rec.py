...
from ppocr.utils.rtl import reverse_reversed_rtl_text
...
class TextRecognizer(object):
    def __init__(self, args, logger=None):
        ...
        self.reverse_rtl_text = args.reverse_rtl_text
        if args.benchmark:
            import auto_log
    ...
    def _normalize_rec_result(self, rec_result):
        if not self.reverse_rtl_text:
            return rec_result
        ...
        for item in rec_result:
            if (
                ...
                item_values[0] = reverse_reversed_rtl_text(item_values[0])
                normalized_result.append(
                    tuple(item_values) if isinstance(item, tuple) else item_values
                )
            else:
    ...
    def __call__(self, img_list):
        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            if self.postprocess_params["name"] == "CTCLabelDecode":
                ...
                rec_result = self.postprocess_op(preds)
            rec_result = self._normalize_rec_result(rec_result)
