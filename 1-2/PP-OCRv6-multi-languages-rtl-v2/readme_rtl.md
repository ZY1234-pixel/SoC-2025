...
本版重点解决 RTL 语言与英文、数字混排时的训练标签顺序和推理后处理问题，同时通过丰富的背景纹理、纸张质感、颜色扰动、模糊、噪声和轻微干扰线等合成手段增强数据多样性，提升模型在复杂场景下的泛化能力。训练阶段标签采用适合 PaddleOCR 从左到右学习图像特征的视觉顺序；测试阶段再通过后处理将识别结果转换为与 `label.txt` 一致的逻辑文本顺序。
...
python tools/train.py \
  -c configs/rec/PP-OCRv6/PP-OCRv6_small_rec_mixed2_richbg_finetune.yml
```
...
本次训练输出目录：
```text
output/mixed2_small_rec_richbg/
```
...
best_epoch: 12
acc: 0.9375999990624
norm_edit_dis: 0.9965053850991477
fps: 1291.226117203679
```
...
python tools/eval.py \
  -c configs/rec/PP-OCRv6/PP-OCRv6_small_rec_mixed2_richbg_finetune.yml \
  -o Global.checkpoints=./output/mixed2_small_rec_richbg/best_accuracy
```
...
python tools/eval.py \
  -c configs/rec/PP-OCRv6/PP-OCRv6_small_rec_mixed2_richbg_finetune.yml \
  -o Global.checkpoints=./output/mixed2_small_rec_richbg/best_model/model
```
...
python tools/export_model.py \
  -c configs/rec/PP-OCRv6/PP-OCRv6_small_rec_mixed2_richbg_finetune.yml \
  -o Global.pretrained_model=./output/mixed2_small_rec_richbg/best_accuracy \
     Global.save_inference_dir=./output/mixed2_small_rec_richbg/inference_model
```

导出后的推理模型目录：
```text
output/mixed2_small_rec_richbg/inference_model/
├── inference.json
├── inference.pdiparams
└── inference.yml
```
...
python tools/infer/predict_rec_rtl.py \
  --use_gpu False \
  --rec_model_dir ./output/mixed2_small_rec_richbg/inference_model \
  --rec_char_dict_path ./data/mixed2_data_richbg/dict_no_space.txt \
  --image_dir ./test-images/single2_en \
  --label_path ./test-images/single2_en/label.txt \
  --save_res_path ./output/mixed2_small_rec_richbg/predicts_single2_en_rtl_postprocess.txt \
  --terminal_order logical
```

测试从 `images_and_label` 中挑选的 200 张未参与训练样本：
```bash
python tools/infer/predict_rec_rtl.py \
  --use_gpu False \
  --rec_model_dir ./output/mixed2_small_rec_richbg/inference_model \
  --rec_char_dict_path ./data/mixed2_data_richbg/dict_no_space.txt \
  --image_dir ./output/mixed2_small_rec_richbg/images_and_label_holdout200/images \
  --label_path ./output/mixed2_small_rec_richbg/images_and_label_holdout200/label_logical.txt \
  --save_res_path ./output/mixed2_small_rec_richbg/images_and_label_holdout200/predicts_holdout200_rtl_postprocess.txt \
  --terminal_order logical
```

对应的可视化结果目录：
```text
output/mixed2_small_rec_richbg/images_and_label_holdout200/vis/
```
...
- `use_space_char` 当前为 `true`，对应训练配置使用 `dict_no_space.txt`，推理时也建议保持一致。
