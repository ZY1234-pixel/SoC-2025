...
class PageDecoder(object):
    ...
    def decode(self, output, pred_read, pred_start, pred_end, img_shape, return_inter_path=False):
        ...
        if line_results:  # 避免空结果报错
            # 提取每个行的垂直坐标（gy）：优先用行内所有元素的gy均值，更鲁棒
            line_gy = []
            for line in line_results:
                line = np.array(line)
                line_cors_gy = cors[line, 1]  # cors[:,1] 是gy（垂直坐标）
                line_gy.append(np.mean(line_cors_gy))
            line_gy = np.array(line_gy)
            # 按gy升序排序（y越小，行在页面上越靠上）
            sorted_indices = np.argsort(line_gy)
            # 重新排列line_results
            line_results = [line_results[i] for i in sorted_indices]
