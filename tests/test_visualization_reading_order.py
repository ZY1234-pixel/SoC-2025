...
from docflow.utils.visualization import draw_reading_order_comparison, draw_reading_order_map
...
def test_draw_reading_order_comparison_concatenates_panels():
    ...
    assert vis.shape[1] == image.shape[1] * 2 + 24
...
def test_draw_reading_order_map_keeps_overlapped_spanning_border_visible():
    ...
    assert np.any(vis[60:90, 18:26] != 255)
    # The local block should also remain visible in the overlap region.
    assert np.any(vis[92:110, 20:35] != 255)
