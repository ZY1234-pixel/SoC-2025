...
def main(args):
    ...
    save_intermediate = getattr(args, "save_intermediate_results", False)
    ...
    for i, image_file in enumerate(image_file_list):
        ...
        for index, img in enumerate(imgs):
            # 分离推理输入与可视化/恢复底图，避免中间过程意外改写同一对象
            raw_img = img.copy()
            infer_img = img.copy()

            res, time_dict = structure_sys(infer_img, img_idx=index)
            ...
            if structure_sys.mode == "structure" and res != []:
                if save_intermediate:
                    draw_img = draw_structure_result(raw_img, res, args.vis_font_path)
                ...
                if save_intermediate:
                    if structure_sys.kie_predictor.predictor is not None:
                        draw_img = draw_re_results(raw_img, res, font_path=args.vis_font_path)
                    else:
                        draw_img = draw_ser_results(raw_img, res, font_path=args.vis_font_path)
                    ...
                    with open(
                        ...
                        f.write(res_str)
            if res != [] and save_intermediate:
                cv2.imwrite(img_save_path, draw_img)
                logger.info("result save to {}".format(img_save_path))
            if args.recovery and res != []:
                from ppstructure.recovery.recovery_to_doc_new import (
                    sorted_layout_boxes,
                    convert_info_docx,
                    draw_sorted_layout,
                )
                ...
                if save_intermediate:
                    # 仅保留一张排序可视化图，统一放在 debug 子目录
                    if index == 0:
                        debug_img = draw_sorted_layout(raw_img, res)
                        cv2.imwrite(os.path.join(debug_dir, "sorted.jpg"), debug_img)
